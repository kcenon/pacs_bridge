...
| **컴파일러** | C++23 (GCC 13+, Clang 16+, MSVC 2022 v17.0+) |
| **메모리** | 최소 2 GB, 권장 8 GB |
...
#### SRS-EMR-001: FHIR R4 클라이언트
| 속성 | 값 |
|-----------|-------|
| **ID** | SRS-EMR-001 |
| **제목** | FHIR R4 HTTP 클라이언트 |
| **설명** | 시스템은 EMR 시스템과의 아웃바운드 HTTP 통신을 위한 FHIR R4 클라이언트를 구현해야 하며, Bundle 연산 및 연결 풀링을 지원해야 한다. |
| **우선순위** | 권장 |
| **단계** | 5 |
| **추적** | FR-6.1.1 - FR-6.1.4 |

**인수 기준:**
1. FHIR RESTful HTTP 요청 수행 (GET, POST, PUT)
2. Bundle 트랜잭션 및 배치 연산 지원
3. 영구 연결을 위한 연결 풀링 구현
4. OAuth2 토큰 기반 인증 지원
5. FHIR OperationOutcome 오류 응답 처리

---

#### SRS-EMR-002: 환자 조회 서비스
| 속성 | 값 |
|-----------|-------|
| **ID** | SRS-EMR-002 |
| **제목** | EMR 환자 조회 |
| **설명** | 시스템은 FHIR Patient 리소스 검색을 통해 외부 EMR 시스템에서 환자 인구통계를 조회해야 하며, 결과 캐싱 및 환자 매칭 로직을 지원해야 한다. |
| **우선순위** | 권장 |
| **단계** | 5 |
| **추적** | FR-6.2.1 - FR-6.2.4 |

**인수 기준:**
1. 환자 식별자(MRN, 주민번호)로 EMR 조회
2. FHIR Patient 리소스 응답 파싱
3. 구성 가능한 환자 매칭 알고리즘 구현
4. 구성 가능한 TTL로 조회 결과 캐싱
5. 부분 매칭 및 모호한 결과 처리

---

#### SRS-EMR-003: 결과 전송 서비스
| 속성 | 값 |
|-----------|-------|
| **ID** | SRS-EMR-003 |
| **제목** | EMR 결과 전송 |
| **설명** | 시스템은 영상의학 리포트 및 스터디 결과를 FHIR DiagnosticReport 리소스로 외부 EMR 시스템에 전송해야 한다. |
| **우선순위** | 권장 |
| **단계** | 5 |
| **추적** | FR-6.3.1 - FR-6.3.4 |

**인수 기준:**
1. 내부 리포트 데이터로 FHIR DiagnosticReport 생성
2. EMR 엔드포인트에 DiagnosticReport 전송
3. 실패 시 재시도를 통한 전달 상태 추적
4. Encounter 컨텍스트 및 환자 참조 포함
5. 예비 및 최종 리포트 상태 지원

---

#### SRS-EMR-004: EMR 어댑터 패턴
| 속성 | 값 |
|-----------|-------|
| **ID** | SRS-EMR-004 |
| **제목** | 추상 EMR 어댑터 인터페이스 |
| **설명** | 시스템은 플러거블 아키텍처를 통해 여러 EMR 시스템 유형을 지원하기 위한 추상 EMR 어댑터 인터페이스를 정의해야 한다. |
| **우선순위** | 권장 |
| **단계** | 5 |
| **추적** | FR-6.4.1 - FR-6.4.3 |

**인수 기준:**
1. EMR 연산(조회, 전송, 구독)을 위한 추상 인터페이스 정의
2. 기본값으로 제네릭 FHIR 기반 어댑터 구현
3. 배포별 EMR 특화 구성 지원
4. 런타임 EMR 어댑터 선택 허용
5. 헬스체크 및 연결 상태 모니터링 제공
6. 벤더별 구성 지원 (Epic, Cerner, Meditech, Allscripts)

---

#### SRS-EMR-005: Encounter 컨텍스트 서비스
| 속성 | 값 |
|-----------|-------|
| **ID** | SRS-EMR-005 |
| **제목** | 임상 Encounter 컨텍스트 제공자 |
| **설명** | 시스템은 영상의학 결과와 임상 Encounter를 연관시키기 위해 외부 EMR 시스템에서 Encounter/방문 컨텍스트를 조회해야 하며, 캐싱을 통한 FHIR Encounter 리소스 조회를 지원해야 한다. |
| **우선순위** | 권장 |
| **단계** | 5 |
| **추적** | FR-6.3.4 |

**인수 기준:**
1. Encounter ID로 Encounter 조회
2. 방문 번호(선택적 식별자 시스템)로 Encounter 검색
3. 특정 환자의 활성 Encounter 검색
4. 상태, 클래스, 참여자, 위치 포함 FHIR Encounter 리소스 파싱
5. 구성 가능한 TTL로 Encounter 조회 캐싱
6. Encounter 상태 처리 (planned, arrived, in-progress, finished, cancelled)

---

#### SRS-EMR-006: 결과 전달 추적
| 속성 | 값 |
|-----------|-------|
| **ID** | SRS-EMR-006 |
| **제목** | EMR 결과 전달 추적기 |
| **설명** | 시스템은 외부 EMR 시스템에 전송된 결과의 전달 상태를 추적해야 하며, 스터디 UID, Accession Number 또는 리포트 ID로 조회를 지원하고, 구성 가능한 용량 및 TTL 기반 만료를 지원해야 한다. |
| **우선순위** | 권장 |
| **단계** | 5 |
| **추적** | FR-6.3.3 |

**인수 기준:**
1. 스터디 UID, Accession Number, 리포트 ID로 전송된 결과 추적
2. 스터디 인스턴스 UID, Accession Number 또는 리포트 ID로 조회 지원
3. 제출 전 중복 전송 감지
4. 구성 가능한 용량 제한 적용 (기본값: 10,000건)
5. 자동 정리를 통한 TTL 기반 만료 구현 (기본값: 7일)
6. 스레드 안전한 인메모리 구현 제공

---

#### SRS-EMR-007: FHIR DiagnosticReport 빌더
| 속성 | 값 |
|-----------|-------|
| **ID** | SRS-EMR-007 |
| **제목** | FHIR DiagnosticReport 리소스 빌더 |
| **설명** | 시스템은 Fluent 빌더 패턴을 사용하여 내부 스터디 결과로부터 유효한 FHIR R4 DiagnosticReport 리소스를 구성해야 하며, 표준 코딩 시스템(LOINC, SNOMED CT) 검증 및 지원을 포함해야 한다. |
| **우선순위** | 권장 |
| **단계** | 5 |
| **추적** | FR-6.3.1, FR-6.3.2 |

**인수 기준:**
1. 내부 스터디 결과 데이터로 DiagnosticReport 생성
2. 필수 필드 지원: status, code, subject
3. 카테고리(radiology), effective date/time, issued date 포함
4. performer 및 results interpreter 참조 첨부
5. conclusion 텍스트 및 코드화된 conclusion (SNOMED CT) 지원
6. 식별자 포함 (Accession Number, Study Instance UID)
7. Encounter, ImagingStudy, ServiceRequest 참조 연결
8. 직렬화 전 리소스 완전성 검증
...
**설명:** FHIR R4를 통해 외부 EMR 시스템에 영상의학 결과를 전송하고 환자/Encounter 컨텍스트를 조회합니다.

| 속성 | 값 |
|-----------|-------|
| **우선순위** | 권장 |
| **단계** | 5 |
| **SRS 요구사항** | SRS-EMR-001 - SRS-EMR-007, SRS-SEC-006, SRS-SEC-007 |
| **PRD 요구사항** | FR-6.1.x - FR-6.4.x |

**사용 사례:**
1. PACS Bridge가 OAuth2를 통해 EMR에 인증 (SMART on FHIR 탐색)
2. MRN으로 EMR에서 환자 인구통계 조회
3. 구성 가능한 기준으로 시스템 간 환자 매칭
4. 임상 연관을 위한 활성 Encounter 컨텍스트 검색
5. 완료된 스터디로 FHIR DiagnosticReport 생성
6. 전달 추적 및 재시도를 통해 EMR에 결과 전송
7. 감사 및 중복 방지를 위한 전달 상태 추적
...
| SRS-SEC-006 | EMR 엔드포인트용 OAuth2 클라이언트 자격증명 | 토큰 기반 | FR-6.1.4 |
| SRS-SEC-007 | SMART on FHIR 엔드포인트 탐색 | 자동 탐색 | FR-6.1.4 |
...
| FR-6.1.1-4 | SRS-EMR-001, SRS-SEC-006, SRS-SEC-007 | 명세됨 |
| FR-6.2.1-4 | SRS-EMR-002 | 명세됨 |
| FR-6.3.1-2 | SRS-EMR-003, SRS-EMR-007 | 명세됨 |
| FR-6.3.3 | SRS-EMR-006 | 명세됨 |
| FR-6.3.4 | SRS-EMR-005 | 명세됨 |
| FR-6.4.1-3 | SRS-EMR-004 | 명세됨 |
| NFR-1.1-6 | SRS-PERF-001-006 | 명세됨 |
| NFR-2.1-5 | SRS-REL-001-005 | 명세됨 |
| NFR-3.1-4 | SRS-SCALE-001-004 | 명세됨 |
| NFR-4.1-5 | SRS-SEC-001-005 | 명세됨 |
| FR-6.1.4 (OAuth2) | SRS-SEC-006, SRS-SEC-007 | 명세됨 |
...
| SRS-EMR-001 | TC-EMR-001 | 통합 | 계획됨 |
| SRS-EMR-002 | TC-EMR-002 | 통합 | 계획됨 |
| SRS-EMR-003 | TC-EMR-003 | 통합 | 계획됨 |
| SRS-EMR-004 | TC-EMR-004 | 단위 | 계획됨 |
| SRS-EMR-005 | TC-EMR-005 | 통합 | 계획됨 |
| SRS-EMR-006 | TC-EMR-006 | 단위 | 계획됨 |
| SRS-EMR-007 | TC-EMR-007 | 단위 | 계획됨 |
| SRS-SEC-006 | TC-SEC-006 | 통합 | 계획됨 |
| SRS-SEC-007 | TC-SEC-007 | 통합 | 계획됨 |
...
| SRS-EMR-xxx | EMR 통합 | SRS-EMR-001 |
...
EMR 통합 오류 코드: -1000 ~ -1124

EMR 클라이언트 오류 (-1000 ~ -1019):
...
| 2.0.0 | 2026-02-07 | kcenon | C++20→C++23, SRS-EMR-001-004 추가 (5단계 EMR 통합), SRS-PERF-003 분리, 요구사항 수 정합성 추가 |
| 2.1.0 | 2026-02-08 | kcenon | EMR 통합 확장: SRS-EMR-005 (Encounter 컨텍스트), SRS-EMR-006 (결과 추적), SRS-EMR-007 (DiagnosticReport 빌더), SRS-SEC-006/007 (OAuth2/SMART), EMR 오류 코드 레지스트리 (-1000 ~ -1124), Section 5.6 (EMR 통합 기능) 추가 |
